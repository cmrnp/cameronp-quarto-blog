...
You can see we now have four rows for each penguin, one for each type of measurement:

```{r}
head(penguins_long)
```

We can use `group_by()` and `summarise()` from the `dplyr` package to group the rows by outcome, and then "summarise" these groups of rows down to a single row containing a statistical model. This makes use of a couple of R tricks: 'list columns', a data frame column that contains a more complex object than the standard R data types (numeric, character, etc); and the new `pick()` verb which returns a data frame containing selected columns (in this case, all of them) within the group that's being operated on.
...
Ideally we would also do a visual check of model assumptions. One way to do this is something like the code below, which saves a residual plot for each model to an image file, which can be inspected later. It uses the `check_model()` function in the `performance` package to generate the plots. This blog post doesn't show the generated residual plots, but they all look fine.
...
Once we've fitted the models, we can obtain quantities of interest from them. In this example we'll look at estimated means for each species, p-values testing the hypothesis that all species means are equal, and comparisons (differences in means) between all pairs of species.
...
We can use `ggplot()` to present the results visually, for example like this. We can see that there's a huge variation between species in means of all of these measurements.
...
The `gt()` function can be used to produce a nice table of results. In the code shown below I've combined the `lower.CL` and `upper.CL` columns to produce a single column with the confidence interval, and separately specified fewer decimal places for body mass than the other measures. The `group_by()` function before `gt()` results in a table sub-heading for each outcome. You could easily change this to `group_by(species)` to arrange the results by species.
...
We can do similar to produce an overall "F" test for each model, testing the hypothesis that all species have equal means for a particular measure. The `joint_test()` function in `emmeans` does this.
...
And once again, these can comparisons can be plotted or presented in a table. In the plot below I've also added a dotted line indicating zero difference.
