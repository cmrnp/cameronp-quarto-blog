---
title: "Untangling missing data terminology (MAR, MCAR, MNAR) using DAGs"
...
The best method for dealing with missing data depends on the underlying process causing the missingness. There is no single approach that is always the best, and the terminology commonly used to describe missingness doesn't directly relate to what analysis methods perform best.
...
The most common classifications for missing data mechanisms are Missing Completely at Random (MCAR), Missing at Random (MAR), or Missing Not at Random (MNAR), based on concepts first introduced by @rubin_inference_1976. These terms are widespread, but can be confusing when first encountered because they do not mean quite what you might expect them to mean.
...
Data is **Missing Not at Random (MNAR)** if it's not Missing at Random  [@little_statistical_2014, p. 12]. At least that's relatively straightforward. In other words, the probability of data being missing is related to what the unobserved data was. It is often implied that MNAR data is a lost cause for statistical analysis, but there are specific examples of MNAR data where common statistical procedures produce valid results for common questions.
...
## Describing missing data processes using causal diagrams (DAGs)
