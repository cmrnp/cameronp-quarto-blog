...
Missing data refers to data which was intended to have been collected but was not, and is a common scenario in biomedical and social science research. Analysing datasets that have missing data requires extra care and consideration to produce correct results.
...
Data is **Missing at Random (MAR)** if the probability of being missing depends only on data that was observed [@little_statistical_2014, p. 12]. Most commonly this refers to a situation where a variable with incomplete data has probability of being missing which relates to completely-observed variables, but there are other possibilities which fit this definition too[^mardef]. This term is misleading if encountered without context, as a plain-language interpretation suggests it may mean something more like MCAR. However, MCAR is a stricter requirement than MAR. If data is MCAR, it is also considered MAR.

[^mardef]: There are a few subtly different ways of defining this concept mathematically; @seaman_what_2013 goes into the detail for those who are interested.
...
Complete case analysis is unbiased if the data is MCAR. This is a sufficient condition, not a necessary condition [@little_missing_2021]: there are situations where complete case analysis may be valid, or at least provide a valid estimate of a particular quantity of interest, under MAR or MNAR. Depending on the amount of missing data, complete case analysis may be inefficient (i.e., have lower power than other methods) because only cases where no variables have missing values are used in the analysis.

Multiple imputation, inverse probability weighting, likelihood-based methods and full Bayesian methods are unbiased if the data is MAR or MCAR [@little_missing_2021]. Multiple imputation is approximately equivalent to a maximum-likelihood method if the underlying model is the same [@collins_comparison_2001].

In some specific MNAR situations, some of the above methods may still be valid, depending on what you're trying to estimate [@white_bias_2010].
...
The idea of using a DAG to represent missing data assumptions was first introduced in @mohan_graphical_2014. To provide more practical advice for specific scenarios, @moreno-betancur_canonical_2018 introduces "canonical" causal diagrams for missing data in the setting where we want to estimate the relationship between an outcome *Y* and an exposure *X*[^exp], adjusting for some confounding variables which may influence both the outcome and the exposure. The confounding variables may be either completely observed (*Z1*) or have some missing data (*Z2*). There may also be some unknown, unmeasured factors *U* which affect the exposure and the confounders.

[^exp]: That's epidemiologist-speak for the main predictor of interest.
...
```{r dag-mcar, fig.width = 6, fig.height = 4}
dag_mcar <- dagify(
  Y ~ X + Z1 + Z2,
  X ~ Z1 + Z2 + U,
  Z1 ~ U,
...
If you have an m-DAG where a quantity of interest cannot be recovered, you can consider doing a sensitivity analysis using a $\delta$-adjustment. This process is described in popular texts on missing data, e.g. @van_buuren_flexible_2018 sec 9.2 (also [available online](https://stefvanbuuren.name/fimd/sec-sensitivity.html)) or @molenberghs_handbook_2015.

## Applying DAGs to real missing data problems

This section draws heavily from @lee_assumptions_2023 --- especially Figure 1 of that paper --- which provides practical guidance for data analysis using missingness DAGs.
...
Before deciding on an analysis method, you must first determine whether the estimand is recoverable, i.e. whether it is possible to estimate it from the data[^ident]. One possibility is to compare your DAG to the canonical DAGs in @moreno-betancur_canonical_2018 and the recoverability results in Table 1 of that paper.

[^ident]: This step is analogous to determining identifiability in causal inference.
